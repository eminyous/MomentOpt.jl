@testset "Objects" begin
    ...
    @testset "AbstractGMPMeasure" begin
        ...
        λ = AnalyticMeasure(
            ...
            MonomialBasis,
            p -> sum(
                c / (1 + first(exponents(m))) for
                (c, m) in zip(coefficients(p), monomials(p))
            ),
        )
        @test λ == λ
        ...
        @test monomials(maxdegree_basis(μ, 2)) ==
              reverse([2.0 * x^2 - 1.0, x * y, 2.0 * y^2 - 1.0, x, y, 1.0])
        @test monomials(MO.covering_basis(μ, 2)) ==
              polynomial_type(x, Float64)[1.0]
    end
    ...
    @testset "Moment Measure" begin
        ...
        MomentOpt.approx_vrefs(m)[1] = MomentOpt.VarApprox(
            MultivariateMoments.measure(vals, mons),
            nothing,
            nothing,
        )
