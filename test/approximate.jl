function testproblem1(factory)
    ...
    mu = @variable gmp μ Meas(
        [x, y];
        support = @set(x^2 - x^4 >= 0 && 1 - x^2 >= 0 && x - y^2 == 0)
    )
...
function testproblem2(factory)
    ...
    con = @constraint gmp μ + ν ==
                    lebesgue_measure_box(x => [-1, 1], y => [-1, 1])
    set_silent(gmp)
    return gmp, mu, con
end
...
function testproblem3(factory, sparsity)
    ...
    f = x * y + y * z - y^2
    @variable gmp mu Meas(
        [x, y, z],
        support = K,
        scheme = PutinarScheme(; sparsity = sparsity),
    )
...
@testset "Approximate" begin
    ...
    @testset "Schmuedgen" begin
        @testset "Dual Schmuedgen" begin
            ...
            @test size.(primal_justification(mu)) ==
                  [(6, 6), (1,), (3, 3), (3, 3), (3, 3), (6,)]
        end
...
@testset "Post Proc" begin
    ...
    MO.approximation_info(gmp).approx_vrefs[1] = MO.VarApprox(
        MultivariateMoments.Measure(
            [
                0.04301405019233425,
                3.725430564600456e-17,
                0.0944511405218325,
                -3.667541691651414e-17,
                0.20739824193451498,
                0.09445115553035512,
                -6.278937572688163e-17,
                0.207398112278084,
                2.2189511821478325e-17,
                0.20739824605238844,
                6.290839177153608e-18,
                0.4554099572402294,
                0.45540988993201603,
                4.3223432627485784e-17,
                1.0000002747597936,
            ],
            [
                x^4,
