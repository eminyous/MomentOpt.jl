...
function covering_basis(
    ...
    return covering_basis(t, p * _mono_one(variables(t)))
end
...
struct AnalyticMeasure{V<:MP.AbstractVariable,BT<:MB.AbstractPolynomialBasis} <:
       AbstractGMPMeasure
    variables::Vector{V}
...
function AnalyticMeasure(
    variables::Vector{V},
    moment_basis::T,
    moment_function::Function,
) where {V<:MP.AbstractVariable,T<:Type{<:MB.AbstractPolynomialBasis}}
    return AnalyticMeasure(
        variables,
        moment_basis,
        ApproximationFunction(moment_function, variables, moment_basis),
    )
end
...
struct VariableMeasure{
    V<:MP.AbstractVariable,
    S<:AbstractBasicSemialgebraicSet,
    BT<:MB.AbstractPolynomialBasis,
    R<:AbstractApproximationScheme,
} <: AbstractGMPMeasure
    variables::Vector{V}
    bsa_set::S
    approx_basis::Type{BT}
...
) = VariableMeasure(vars, support, basis, scheme)
...
#
function compatible(f1::AnalyticGMPObject, f2::AnalyticGMPObject)
    return variables(f1) == variables(f2) &&
           supertype(typeof(f1)) == supertype(typeof(f2))
end
function Base.sum(v::Vector{<:AnalyticGMPObject})
    ...
    return constructor(typeof(v[1]))(
        variables(v[1]),
        approx_basis(v[1]),
        sum(approx_function.(v)),
    )
end
...
function Base.:*(a::Number, f::AnalyticGMPObject)
    return constructor(typeof(f))(
        variables(f),
        approx_basis(f),
        a * approx_function(f),
    )
...
function prod_meas(μ::AnalyticMeasure, ν::AnalyticMeasure)
    ...
    return AnalyticMeasure(
        vars,
        approx_basis(μ),
        ApproximationFunction(approxfunction, vars, approx_basis(μ)),
    )
end
