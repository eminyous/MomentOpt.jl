...
"""
ZeroMeasure(vars::Vector{<:MP.AbstractVariable}; basis = MonomialBasis) =
    AnalyticMeasure(vars, basis, x -> 0)
...
function DiracMeasure(
    ...
    return AnalyticMeasure(
        vars,
        MonomialBasis,
        x -> prod(point[i]^exponents(x)[i] for i in 1:length(point)),
    )
end
...
function variable_box(args...)
    ...
    return VariableBox(
        Dict(first(a) => convert(Vector{T}, last(a)) for a in args),
    )
end
...
function lebesgue_box(
    ...
    mom = sum(
        c * lebesgue_box(variables_with_domain, m; normalize = normalize)
...
function lebesgue_measure_box(args...; kwargs...)
    return lebesgue_measure_box(variable_box(args...); kwargs...)
end
