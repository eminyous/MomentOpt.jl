function measure_data(m::GMPModel, id::Int)
    ...
    for (i, con) in gmp_constraints(m)
        ...
        elseif !(jump_function(con)[vref] isa Nothing)
            cons[i] = monomials(
                maxdegree_basis(moi_set(con), approximation_degree(m)),
            )
        end
    end
...
function dual_variable(
    model::JuMP.Model,
    con::AbstractGMPConstraint,
    ...
    if con isa MeasureConstraint
        variable = @variable(
            model,
            variable_type = Poly(maxdegree_basis(moi_set(con), deg))
        )
...
function approximate!(model::GMPModel, ::AbstractDualMode)
    # init dual variables
    meas_data =
        Dict(i => measure_data(model, i) for i in keys(gmp_variables(model)))

    dvar = Dict()
    for (i, con) in gmp_constraints(model)
        dvar[i] = dual_variable(
    ...
    PT = polynomial_type(
        eltype(variables(gmp_variables(model)[1].v)),
        JuMP.AffExpr,
    )
    dlhs = Dict{Int,PT}()
    ...
    for (i, v) in gmp_variables(model)
        ...
        just[i] = [
            _fix(
                dual_scheme_variable(approximation_model(model), sp) *
                sp.polynomial,
        ...
        dcon[i] =
            @constraint approximation_model(model) scheme_parts[i].denominator *
                                                   p == 0
    end
            ...
            approx_crefs(model)[i] = ConApprox(
                [
                    integrate(
                        Dict(
                            i => dual(dcon[i]) for
                            i in keys(gmp_variables(model))
                        ),
                        jump_function(con),
                    ) - MOI.constant(moi_set(con)),
                ],
                [value.(dvar[i])],
            )
        else
...
function approximate!(model::GMPModel, ::AbstractPrimalMode)
    ...
    for (i, con) in gmp_constraints(model)
        if shape(con) isa MomentConstraintShape
            cref = @constraint approximation_model(model) integrate(
                pvar,
                jump_function(con),
            ) in moi_set(con)
            pcon[i] = [cref]
        elseif shape(con) isa MeasureConstraintShape
            ...
            mons = monomials(
                maxdegree_basis(
                    approx_basis(refmeas),
                    variables(refmeas),
                    approximation_degree(model),
                ),
            )

            pcon[i] = @constraint approximation_model(model) sum(
                c .* (MM.expectation.(mons, pvar[index(v)])) for
                (c, v) in jump_function(con)
            ) .== integrate.(mons, refmeas)
        end
    end

    # add objective
    @objective approximation_model(model) objective_sense(model) integrate(
        pvar,
    ...
    for i in keys(gmp_variables(model))
        ...
        for (ju, part) in zip(just[i], scheme_parts[i].schemeparts)
            if set_type(part) == PSDCone
                q =
                    dot(
                        monomials(part.monomials),
                        dual(ju) * monomials(part.monomials),
                    ) * part.polynomial
            else
        ...
        approx_vrefs(model)[i] = VarApprox(
            MM.Measure(value.(pvar[i].a), pvar[i].x),
            convert.(Array{Float64}, value.(just[i])),
            p,
        )
    end
    for (i, con) in gmp_constraints(model)
        if shape(con) isa MeasureConstraintShape
            ...
            approx_crefs(model)[i] = ConApprox(
                value.(pcon[i]) .-
                MOI.constant.(JuMP.moi_set.(constraint_object.(pcon[i]))),
                -dot(dual.(pcon[i]), mons),
            )

        else
            approx_crefs(model)[i] = ConApprox(
                value.(pcon[i]) .- MOI.constant(moi_set(con)),
                dual.(pcon[i]),
            )
        end
    end
